======================================================== mapping
cd /work4/home/joweihsieh/20240424_At_meth_ATAC_pub/ChIP_raw/1.raw
ln -s /work4/home/yenmr/project/HDAC6/ChIP/raw/*_trimmed.fastq.gz ./
========================================================
...
ln -s /work4/home/yenmr/project/HDAC6/ChIP/raw/*bam ./



conda activate ATAC
======================================================== bash Step1.idr.sh & #18114
...
do
    sample_name=$(basename "$log_file" | cut -d '_' -f 3,4,7,8)
...
do
    sample_name=$(basename "$log_file" | cut -d '_' -f 2,3,4)
    percentage=$(grep -oP '\d+\.\d+%' "$log_file" )

    echo "Sample: $sample_name, Percentage: $percentage" >> FRiP_percent.txt
done


grep -oP '\d+\.\d+%' ChIP_H3Ac_WT_1_friplog
